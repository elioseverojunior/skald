...
The conformance harness (`tests/yaml_test_suite.rs`) validates Skald against all
735 official YAML test cases:

```bash
cargo test -p skald-yaml-test-suite --test yaml_test_suite
```
...
```bash
cargo test -p skald-yaml-test-suite --test round_trip_fidelity
```
